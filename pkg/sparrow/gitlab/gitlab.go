...
type Gitlab interface {
	...
	FetchFiles(ctx context.Context) ([]types.GlobalTarget, error)
...
type File struct {
	...
	Content       types.GlobalTarget `json:"content"`
...
func (g *Client) FetchFiles(ctx context.Context) ([]types.GlobalTarget, error) {
...
func (g *Client) fetchFile(ctx context.Context, f string) (types.GlobalTarget, error) {
	log := logger.FromContext(ctx).With("file", f)
	var res types.GlobalTarget
	// URL encode the name
	n := url.PathEscape(f)
	req, err := http.NewRequestWithContext(ctx,
		http.MethodGet,
